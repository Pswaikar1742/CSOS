...
import { MOCK_TRUCKS, MOCK_COMPLAINTS, FLEET_SUMMARY } from '@/lib/mock-fleet-data';
...
export default function SanitationCommandCenter() {
  const { incidents, neuralLogs } = useCSOSSocket('sanitation');
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <TopNav role="sanitation" alertCount={MOCK_COMPLAINTS.filter(c => c.status !== 'resolved').length} />

      {/* Summary Cards Row */}
      <div className="shrink-0 px-4 py-3 bg-white border-b border-slate-200">
        <div className="grid grid-cols-4 gap-3">
          <StatsCard title="Waste Collected" value={FLEET_SUMMARY.wasteCollected} icon={<Recycle className="h-4 w-4" />} variant="sanitation" />
          <StatsCard title="Routes Completed" value={FLEET_SUMMARY.routesCompleted} icon={<Route className="h-4 w-4" />} variant="sanitation" trend="up" trendLabel="90%" />
          <StatsCard title="Workers Present" value={FLEET_SUMMARY.workersPresent} icon={<Users className="h-4 w-4" />} variant="sanitation" />
          <StatsCard title="Complaints" value={FLEET_SUMMARY.complaintsToday} icon={<MessageSquare className="h-4 w-4" />} variant="sanitation" />
        </div>
      </div>
      ...
      <div className="flex-1 flex gap-3 p-3 overflow-hidden">
        ...
        <section className="w-1/5 rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-slate-200">
            ...
            <div className="mt-1 flex gap-2 text-[10px] font-semibold">
              <span className="flex items-center gap-1"><StatusDot status="available" size="sm" /> {MOCK_TRUCKS.filter(t => t.status === 'active').length}</span>
              <span className="flex items-center gap-1"><StatusDot status="idle" size="sm" /> {MOCK_TRUCKS.filter(t => t.status === 'idle').length}</span>
              <span className="flex items-center gap-1"><StatusDot status="offline" size="sm" /> {MOCK_TRUCKS.filter(t => t.status === 'offline').length}</span>
            </div>
          </div>
        ...
        <section className="w-1/5 rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-slate-200">
            ...
            <p className="text-[10px] text-slate-500">{MOCK_COMPLAINTS.filter(c => c.status !== 'resolved').length} active</p>
          </div>
            ...
            {MOCK_COMPLAINTS.map((complaint) => (
              <article key={complaint.id} className={`rounded-lg border p-3 ${complaint.status === 'resolved' ? 'border-slate-200 bg-slate-50 opacity-60' : 'border-slate-200 bg-white'}`}>
                ...
                {complaint.status === 'registered' && (
                  <button className="mt-2 w-full py-2 rounded-md bg-amber-500 text-white text-xs font-bold hover:bg-amber-600 transition min-h-[40px]">
